...
const colDetector = (i) => {
    if (!gOver && gState === 1) {
        ...
        const birdLeft = cords.birdX;
        const birdRight = cords.birdX + size.birdW;
        const birdTop = cords.birdY;
        const birdBottom = cords.birdY + size.birdH;
...
if (gState === 0) {
    ...
    cx.fillText("\nPress R to restart the game.", 4, size.sh / 2 + 25);
}
